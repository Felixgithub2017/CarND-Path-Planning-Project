...
double theta, const vector<double> &maps_x, const vector<double> &maps_y)
{
...
int main()
{
  ...
  h.onMessage([&max_speed, &speed_diff, &ref_vel, &lane, 
        ...
        if (event == "telemetry")
        {
		   ...
		   //Check if there is a car in the front, left or right
            ...
            for(int i=0; i < sensor_fusion.size(); i++)
            {
				...
				/*Currently we have only three lanes 
                and each lane has 4 meter width. 
				*/
				if(d > 0 && d < 4)
                {
					check_car_lane = 0;
                ...
                {
					//A vehicle is on the same lane and 
                    //check if the car is in front of the ego car
					car_ahead |= check_car_s > car_s && (check_car_s - car_s) < 18;

				} else if((check_car_lane - lane) == -1)
                {
					//A vehicle is on the left lane and check that is in certain range
					//car_left |= (car_s+30) > check_car_s  && (car_s-30) < check_car_s;
                    car_left |= (car_s+15) > check_car_s  && (car_s-8) < check_car_s;

				} else if((check_car_lane - lane) == 1)
                {
					//A vehicle is on the right lane and check that is in certain range
					//car_right |= (car_s+30) > check_car_s  && (car_s-30) < check_car_s;
                    car_right |= (car_s+15) > check_car_s  && (car_s-8) < check_car_s;
							
				}
			}
//------------------------------------------------------------------------------------
            //control module, perform lane changing, distance keeping, speed control
            if(car_ahead)
            {
                if(lane == 0)
                {
...
//---------------------------------------------------------------------------------
			...
			//For the smooth transition, add previous path points
